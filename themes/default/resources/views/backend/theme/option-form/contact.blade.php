...
{{-- Restricted Domains Field Start --}}
<div class="form-group row py-3 border-bottom">
    <div class="col-xl-4">
        <label for="restricted_domains" class="font-16 bold black">{{ translate('Restricted Domains') }}
        </label>
        <span class="d-block">{{ translate('Add a list of email providers for demo restriction. Emails with these providers cannot be used for demo request.') }}</span>
    </div>
    <div class="col-xl-6 offset-xl-1">
        <textarea 
            ...
            rows="4"
            placeholder="{{ translate('ex: gmail.com, outlook.com, hotmail.com') }}">{{ isset($option_settings['restricted_domains']) ? $option_settings['restricted_domains'] : '' }}</textarea>
        <small>{{ translate('Use a comma separator to enter more than one domain.') }}</small>
    </div>
</div>
{{-- Restricted Domains Field End --}}
